...
| R3 | Correlation IDs generated/validated truthfully on the event trace-id vocabulary; documented response header echo | `resolveCorrelation` accepts inbound `X-Request-ID` iff it satisfies the shared grammar ([A-Za-z0-9._~:@-], ≤128), else generates via `domain.NewID(PrefixRequest)`; source logged (`header`\|`generated`); effective id echoed; entropy failure logs Error and marks every issued id `req_generation_failed`; `PrefixRequest` added to the contractual prefix table | `TestCorrelationIDAcceptanceGrammar` (10 cases end-to-end incl. injection shapes through a real HTTP client); `TestValidCorrelationID`, `TestResolveCorrelationTruthfulSources`; smoke asserts echoed id equals logged id |
