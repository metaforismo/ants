...
-- their configured horizon, oldest-terminal-first. Each partial index covers
-- exactly one eligible class in the order the victim scan requests, so
-- bounded rounds never scan live work (pending/leased/dead are outside the
-- predicates by construction). No table columns change.
